...
struct X86RegAllocState {
    ...
    void load_from_spill(std::ostream& out, const Operand& op,
                         std::string reg) {
        if (op.type.isUnsigned) {
            switch (op.type.bytes) {
                case 1:
                    out << "  movzx " << reg << ", BYTE PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
                case 2:
                    out << "  movzx " << reg << ", WORD PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
                case 4:
                    out << "  mov " << get_reg_32(reg) << ", DWORD PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
                case 8:
                    out << "  mov " << reg << ", QWORD PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
            }
        } else {
            switch (op.type.bytes) {
                case 1:
                    out << "  movsx " << reg << ", BYTE PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
                case 2:
                    out << "  movsx " << reg << ", WORD PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
                case 4:
                    out << "  movsxd " << reg << ", DWORD PTR [rbp - "
                        << temp_to_stack_offset[op.temp_id] << "]" << std::endl;
                    break;
                case 8:
                    out << "  mov " << reg << ", QWORD PTR [rbp - "
...
class X86Generator {
    ...
    std::string activateDst(const Operand& op,
                            X86RegAllocState& regalloc_state);
    std::string activateSrc1(const Operand& op,
                             X86RegAllocState& regalloc_state);
    std::string activateSrc2(const Operand& op,
                             X86RegAllocState& regalloc_state);
  ...
  public:
    X86Generator(const IRProgram& program, ErrorReporter& reporter,
