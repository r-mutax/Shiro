...
IRFunction IRGenerator::gen_function(ASTNode* node) {
    ...
    FunctionDefinitionNode* func_def =
        static_cast<FunctionDefinitionNode*>(node);
    IRFunction func;
...
Operand IRGenerator::gen_stmt(ASTNode* node) {
    if (node->kind == ASTNode::NODE_EXPRESSION_STATEMENT) {
        ExpressionStatementNode* stmt =
            static_cast<ExpressionStatementNode*>(node);
...
Operand IRGenerator::gen_expr(ASTNode* node) {
    if (node->kind == ASTNode::NODE_INTEGER) {
        ...
        Operand res_temp =
            Operand::Temp(next_temp++, if_node->then_block->evaluated_type);
        ...
        int end_label = next_label++;
        Operand res_temp =
            Operand::Temp(next_temp++, while_node->evaluated_type);
        ...
        if (bin_op->op.type == Token::AND_AND) {
            ...
            int end_label = next_label++;
            Operand res_temp =
                Operand::Temp(next_temp++, bin_op->evaluated_type);
            ...
            switch (bin_op->op.type) {
                ...
                default:
                    error(bin_op->loc,
                          "Unexpected operator: " + bin_op->op.to_str());
            }
            return ret;
        }
    }
...
void IRFunction::constructCFG(const std::vector<IRInstruction>& instructions) {
    ...
    for (size_t i = 0; i < instructions.size(); ++i) {
        ...
        if (instr.op == IRInstruction::JMP || instr.op == IRInstruction::JZ ||
            instr.op == IRInstruction::JNZ || instr.op == IRInstruction::RET) {
            is_leader = true;
        }
    }
