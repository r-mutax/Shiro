...
std::string X86Generator::activateDst(const Operand& op,
                                      X86RegAllocState& regalloc_state) {
    ...
    switch (op.kind) {
        ...
        case Operand::TEMP: {
            if (regalloc_state.is_spill(op)) {
                ...
                return regalloc_state
                    .free_regs[regalloc_state.temp_to_reg[op.temp_id]]
                    .first;
            }
        }
        case Operand::LLABEL: {
...
std::string X86Generator::activateSrc1(const Operand& op,
                                       X86RegAllocState& regalloc_state) {
    ...
    switch (op.kind) {
        ...
        case Operand::TEMP: {
            if (regalloc_state.is_spill(op)) {
                regalloc_state.load_from_spill(out, op,
                                               regalloc_state.SpillSrc1Reg);
...
std::string X86Generator::activateSrc2(const Operand& op,
                                       X86RegAllocState& regalloc_state) {
...
void X86Generator::deactivateDst(const Operand& op,
                                 X86RegAllocState& regalloc_state) {
    ...
    if (regalloc_state.is_spill(op)) {
        switch (op.type.bytes) {
            case 1:
                out << "  mov BYTE PTR [rbp - "
                    << regalloc_state.temp_to_stack_offset[op.temp_id] << "], "
                    << regalloc_state.get_reg_8(regalloc_state.SpillDstReg)
                    << std::endl;
                break;
            case 2:
                out << "  mov WORD PTR [rbp - "
                    << regalloc_state.temp_to_stack_offset[op.temp_id] << "], "
                    << regalloc_state.get_reg_16(regalloc_state.SpillDstReg)
                    << std::endl;
                break;
            case 4:
                out << "  mov DWORD PTR [rbp - "
                    << regalloc_state.temp_to_stack_offset[op.temp_id] << "], "
                    << regalloc_state.get_reg_32(regalloc_state.SpillDstReg)
                    << std::endl;
                break;
            case 8:
        ...
        std::string reg =
            regalloc_state.free_regs[regalloc_state.temp_to_reg[op.temp_id]]
                .first;
        if (op.type.isUnsigned) {
            switch (op.type.bytes) {
                case 1:
                    out << "  movzx " << reg << ", "
                        << regalloc_state.get_reg_8(reg) << std::endl;
                    break;
                case 2:
                    out << "  movzx " << reg << ", "
                        << regalloc_state.get_reg_16(reg) << std::endl;
                    break;
                case 4:
                    out << "  mov " << regalloc_state.get_reg_32(reg) << ", "
                        << regalloc_state.get_reg_32(reg) << std::endl;
                    break;
                case 8:
                    break;
                default:
    ...
    switch (instr.op) {
        ...
        case IRInstruction::Op::CALL: {
            for (size_t i = 0; i < instr.args.size(); ++i) {
                if (i < 6) {
                    std::string reg =
                        activateSrc1(instr.args[i], regalloc_state);
                    out << "  mov " << argregs[i] << ", " << reg << std::endl;
                } else {
                    fatal("Function with more than 6 arguments not supported "
                          "yet");
                }
            }
