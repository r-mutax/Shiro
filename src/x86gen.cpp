...
std::string X86Generator::to_x86(const Operand& op, X86RegAllocState& regalloc_state) {
    switch (op.kind) {
...
void X86Generator::gen_instruction(const IRInstruction &instr, X86RegAllocState& regalloc_state) {
  switch (instr.op) {
  case IRInstruction::Op::MOV:
    out << "  mov rax, " << to_x86(instr.src1, regalloc_state) << std::endl;
    out << "  mov " << to_x86(instr.dst, regalloc_state) << ", rax" << std::endl;
    break;
  case IRInstruction::Op::ADD:
    if(!regalloc_state.is_same_temp(instr.src1, instr.dst)){
      out << "  mov " << to_x86(instr.dst, regalloc_state) << ", " << to_x86(instr.src1, regalloc_state) << std::endl;
    }
    out << "  add " << to_x86(instr.dst, regalloc_state) << ", " << to_x86(instr.src2, regalloc_state) << std::endl;
    break;
  case IRInstruction::Op::SUB:
    if(!regalloc_state.is_same_temp(instr.src1, instr.dst)){
      out << "  mov " << to_x86(instr.dst, regalloc_state) << ", " << to_x86(instr.src1, regalloc_state) << std::endl;
    }
    out << "  sub " << to_x86(instr.dst, regalloc_state) << ", " << to_x86(instr.src2, regalloc_state) << std::endl;
    break;
  case IRInstruction::Op::MUL:
    if(!regalloc_state.is_same_temp(instr.src1, instr.dst)){
      out << "  mov " << to_x86(instr.dst, regalloc_state) << ", " << to_x86(instr.src1, regalloc_state) << std::endl;
    }
    out << "  imul " << to_x86(instr.dst, regalloc_state) << ", " << to_x86(instr.src2, regalloc_state) << std::endl;
    break;
  ...
  case IRInstruction::Op::MOD:
    ...
    out << "  mov " << to_x86(instr.dst, regalloc_state) << ", rdx" << std::endl;
    break;
