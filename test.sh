...
assert "fn inc(r: &i8) -> i8 { r = r + 1; return r; } fn main() -> i8 { let x: i8 = 41; inc(&x); return x; }" 42
assert "struct Point { x: i8, y: i8 }; fn sum(p: &Point) -> i8 { return p.x + p.y; } fn main() -> i8 { let p: Point; p.x = 10; p.y = 32; return sum(&p); }" 42
